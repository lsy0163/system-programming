...
/* Shuffles each element randomly */
void 
list_shuffle (struct list *list)
{
  srand(time(NULL));
  size_t size = list_size(list);
  struct list_elem *pa, *pb;

  for (pa = list_begin(list); pa != list_end(list); pa = list_next(pa))
    {
      for (pb = list_next(pa); pb != list_end(list); pb = list_next(pb))
        {
          int temp = rand() % 2; // 0 or 1
          if (temp) list_swap(pa, pb);
        }
    }
}
  ...
  while (!list_empty(list))
    {
      ...
      free(list_entry(temp, struct list_item, elem));
    }
}
...
/* Return the list's data located in "index" of the list */
struct list_elem *
list_access (struct list *list, int index)
{
  if (!list_empty (list))
    {
      if (0 <= index && index <= (list_size (list) - 1))
        {
          struct list_elem *e;
          int count = 0;
          for (e = list_begin (list); e != list_end (list); e = list_next (e))
