...
/* Prints bitmap */
void
bitmap_print (const struct bitmap *b)
{
  if (b != NULL)
  {
...
/* Create new bitmap bm with size of (bitmap_size(b) + size)
   and returns the pointer of bitmap. */
struct
bitmap *bitmap_expand (struct bitmap *b, int size)
{
  struct bitmap *bm; // new bitmap
  bm = bitmap_create(size + bitmap_size(b));
  if (bm != NULL)
    {
      for (size_t i = 0; i < bitmap_size(b); i++) 
        {
          if (bitmap_test(b, i)) bitmap_mark (bm, i);
        }
      return bm;
    }
  return NULL;
